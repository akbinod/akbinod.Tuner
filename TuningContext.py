...
class TuningContext:
    def __init__(self, ui, config, func_main, func_downstream):
        '''
        tuner: set to self
        ...
        self.__cb_main = func_main
        self.__cb_downstream = func_downstream
    ...
    def on_exit_carousel(self, carousel):
        try:
            ...
            # way to capture results/issues with the very
            # last image displayed if there is no tagging
            # by the user.
    ...
    def invoke(self, *, pass_ui=True):
        ...
        def invoke_main():
            ...
            safe_invoke(self.__cb_main, self.func_name)
            # done calculating - show the results
            img = self.carousel.user_image_main
            # grid_searcehs are usually headless
            if not img is None:
                img = self.__insert_thumbnail(img, self.carousel.tn_main)
                # show the main image here
                self.ui.on_show_main(img)
            return

        def invoke_downstream():
            #  call the user proc that does the secondary processing/application
            self.__calling_main = False
            safe_invoke(self.__cb_downstream, self.func_name_down)
            img = self.carousel.user_image_down
            if not img is None:
                # show it in the secondary window
                img = self.__insert_thumbnail(img, self.carousel.tn_down)
                self.on_show_downstream(img)
            return
        ret = True
        try:
            ...
            self.on_show_results(self.results)
        except:
            pass
        finally:
            # last thing
    ...
    @property
    def func_name(self):
        return self.get_func_name(self.__cb_main)

    @property
    def func_name_down(self):
        return self.get_func_name(self.__cb_downstream)
    ...
    def save_last_invocation(self):
        ...
        if save:
            ...
            hive = None
            if self.carousel.title in self.carousel_data:
                hive = self.carousel_data[self.carousel.title]
            else:
    ...
    def before_invoke(self):
        ...
        # get updated args
        self.invocation.args = self.ui.arg_mgr.gather()
    ...
    def image(self):
        ...
        '''
        return np.copy(self.carousel.image)

    @image.setter
    def image(self, val):
        if self.__calling_main:
            self.carousel.user_image_main = val
        else:
            # the object setting this is the downstream func
            self.carousel.user_image_down = val
    ...
    def get_temp_file(self, suffix=".png"):
        ...
        if self.carousel.file_count <= 1 or suffix.endswith(".png"):
            # results for a single file, or it's
            # an image file being saved.
            # Use the file name
            it = self.carousel.title
        else:
    ...
    def save_image(self):
        ...
        cv2.imwrite(fname,self.carousel.user_image_main)
        if not self.carousel.user_image_down is None:
            fname = fname.replace(".main.", ".downstream.")
            cv2.imwrite(fname,self.carousel.user_image_down)
        return
    ...
    def thumbnail(self):
        ...
        '''
        if self.__calling_main:
            return self.carousel.tn_main
        else:
            return self.carousel.tn_down

    @thumbnail.setter
    def thumbnail(self,val):
        # this is the current thumbnail
        if self.__calling_main:
            self.carousel.tn_main = val
        else:
            self.carousel.tn_down = val
        return
    ...
    def review(self,carousel):
        self.headless = False
        with Carousel(self,carousel) as tray:
            for slot in tray:
                ret  = self.invoke(pass_ui=False)
                if not ret: break
        return ret

    def grid_search(self, carousel, headless, esc_cancels_carousel = False):
        self.headless = headless
        # the ranges that our trackbars have
        ranges, keys = self.ui.arg_mgr.get_ranges()
        with Carousel(self,carousel) as tray:
            for slot in tray:
                # Bang on this image.
                ...
                for t in cart:
                    ...
                    self.ui.arg_mgr.set_values_headless(t, keys)
                    # Invoke target
